...
# Blueprint ─ el nombre interno y la variable son “exportaciones”
exportaciones_bp = Blueprint(
    "exportaciones",          # nombre del blueprint
    __name__,
    url_prefix="/exportar"    # prefijo de las rutas
)

# ---------- Exportar a Excel ----------
@exportaciones_bp.route("/excel")
@login_required
def exportar_excel():
    ...
    df_ingresos = pd.DataFrame(
        [{"Monto": i.monto, "Categoría": i.categoria, "Fecha": i.fecha} for i in ingresos]
    )
    df_gastos = pd.DataFrame(
        [{"Monto": g.monto, "Categoría": g.categoria, "Fecha": g.fecha} for g in gastos]
    )
